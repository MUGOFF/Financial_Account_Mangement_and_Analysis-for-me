...
class TransactionUserSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = Transaction
        exclude = ('written_datetime', 'updated_datetime', 'transaction_from', 'transaction_from_card')
        
    def get_target_nickname(self, obj):
        owner = obj.transaction_from.owner
        settings = Company_Category_Correlation.objects.filter(owner = owner)
        if nickname:
            return nickname.x
        return None
